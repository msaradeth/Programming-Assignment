...
public class ProductAdapter2 extends BaseAdapter {
	...
	private ProductTwoColumn twoColproduct;
	private Activity activity;
	private LayoutInflater layoutInflater;
	private List<ProductTwoColumn> alProduct;
	private ImageCache imageCache;
	private String action;
	private int myPosition;	
	
	//Constructor
	public ProductAdapter2(Activity activity, LayoutInflater layoutInflater, List<ProductTwoColumn> alProduct, OnClickListener callback, String callFrom, String action, ImageCache imageCache) {		
...
/*			//Set a click listener callback 
			if (callFrom.equalsIgnoreCase("CreateProductActivity") && action.equalsIgnoreCase("createProduct")) {
		...
		twoColproduct = alProduct.get(position);
    	...
    	for (int ii=0; ii<alHolder.size(); ii=ii+1) { 
       		...
       		switch (ii) {
       			case 0:
       				product = twoColproduct.col1;
       				break;
       			case 1: 
       				product = twoColproduct.col2;
       				break;       				
       		}      
       		if (product != null) {
           		holder.url = product.getImageUrl();
           		//Display data to view
           		displayData(holder, product, twoColproduct);         			
       		}
	...
	//Display data to view
	public void displayData(ViewHolder holder, Product product, ProductTwoColumn twoColproduct) {
    	...
    	//Draw from memory cache if exist else load product image from asset folder and cache it
   		...
   		final Product tmpProduct = twoColproduct.col1; 
        ...
        final Product tmpProduct2 = twoColproduct.col2;
